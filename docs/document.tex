...
This work evaluates LSR through two complementary approaches: (1) controlled experiments on synthetic embeddings with parameterized density collapse, which isolate the geometric properties LSR is designed to exploit, and (2) preliminary evaluation on HotpotQA \cite{yang2018hotpotqa} using sentence-transformer embeddings, which tests the method on real retrieval neighborhoods. Together, these experiments aim to determine whether LSR reduces redundancy within retrieved neighbors, whether variance-aware selection produces more diverse evidence, and how LSR compares to top-$k$ retrieval and MMR across different neighborhood geometries.

\subsection{Synthetic Evaluation}

The primary evaluation uses synthetic embedding neighborhoods with controlled density collapse. This approach allows systematic variation of key parameters---collapse strength ($r_1$), neighborhood size ($|\mathcal{N}|$), embedding dimension ($d$), and cluster structure---that are difficult to isolate in real corpora.

\paragraph{Setup.} Synthetic neighborhoods are generated in $d = 256$ dimensions. A query embedding is sampled uniformly on the unit sphere. Relevant documents are generated along a principal direction with Gaussian noise perpendicular to this axis; the ratio of along-axis to perpendicular variance controls the collapse factor. Documents are organized into $c \in \{2, 3, 4, 5\}$ clusters along the principal direction, simulating paraphrase groups. Noise documents are sampled uniformly on the sphere.

\paragraph{Baselines.} LSR is compared against three baselines:
\begin{itemize}
    ...
    \item \textbf{Random Sampling}: Uniform random selection from the threshold neighborhood.
\end{itemize}

\paragraph{Metrics.}
\begin{itemize}
    \item \textbf{Redundancy}: Average pairwise cosine similarity within the selected set (lower is better).
    \item \textbf{Cluster Coverage}: Number of distinct document clusters represented in the selected set.
    \item \textbf{Variance Coverage}: Fraction of local PCA variance captured by the selected subset.
\end{itemize}

\subsection{Preliminary Benchmark: HotpotQA}

To validate that the synthetic findings transfer to real embedding neighborhoods, a preliminary evaluation is conducted on HotpotQA \cite{yang2018hotpotqa}, a multi-hop QA dataset where each question requires evidence from multiple supporting documents.

\paragraph{Setup.} A subset of 200 validation questions from HotpotQA (distractor setting) are evaluated. Each question includes 10 context documents, of which 2 contain gold supporting facts. Passages are embedded using \texttt{all-MiniLM-L6-v2} (384-dimensional sentence-transformer embeddings), and all vectors are unit-normalized. LSR, top-$k$, and MMR select $k = 5$ passages per query from the full context set.

\paragraph{Task metric.} In addition to the geometric metrics above, \textbf{supporting fact recall} measures the fraction of gold supporting document titles present in the selected set. This directly measures whether diversity-aware retrieval helps cover the multiple evidence sources required for multi-hop reasoning.

\subsection{Computational Scaling}

A separate timing benchmark measures wall-clock execution time for LSR and MMR as embedding dimension increases from 3 to 1024, on neighborhoods of 500 points with $k = 10$. This isolates the computational advantage of LSR's $O(|N|d)$ PCA-based approach versus MMR's $O(nkd)$ iterative selection.

\subsection{Implementation Details}

For synthetic experiments, embeddings are generated directly in $\mathbb{R}^{256}$ and normalized to unit length. Threshold neighborhoods are constructed with $\tau \in \{0.50, 0.75, 0.85\}$, and the sample size is $k \in \{5, 8, 10\}$. For HotpotQA, all 10 context passages per question form the candidate set (no threshold filtering, as the dataset provides a fixed candidate pool).
...
On synthetic neighborhoods with controlled density collapse, LSR consistently outperforms top-$k$ and random sampling on all geometric metrics. In the strongly collapsed regime ($r_1 \geq 0.70$), LSR achieves the highest cluster coverage (hitting all $c$ planted clusters) and the lowest redundancy, while top-$k$ typically concentrates selections in 1--2 clusters. These gains increase with collapse strength: at $r_1 = 0.95$ (``chaos mode'' in the companion notebook), LSR selects documents from all clusters while top-$k$ collapses onto a single paraphrase group.

MMR \cite{carbonell1998mmr} provides strong diversity across all collapse regimes. In weakly collapsed neighborhoods ($r_1 < 0.50$), MMR matches or slightly exceeds LSR, as its pairwise formulation does not depend on a dominant spectral direction. In strongly collapsed neighborhoods, LSR achieves comparable cluster coverage to MMR at lower computational cost. Adaptive LSR (Section~\ref{sec:adaptive}) captures the best of both: it routes to LSR when collapse is detected and falls back to MMR otherwise.

\paragraph{HotpotQA preliminary results.}
On 200 HotpotQA validation questions, threshold neighborhoods from sentence-transformer embeddings frequently exhibit moderate to strong density collapse. Each multi-hop question requires evidence from two distinct supporting documents; top-$k$ retrieval often selects multiple passages from the same document (high redundancy), while LSR spreads selections across documents, improving supporting fact recall. Detailed results are reported in Table~\ref{tab:hotpotqa} once the full evaluation completes.

\paragraph{Computational scaling.}
The timing benchmark confirms LSR's computational advantage. At 1024 embedding dimensions with 500-point neighborhoods, LSR completes in under 15ms per query while MMR requires over 100ms---a difference that compounds in production retrieval pipelines processing thousands of queries per second.
